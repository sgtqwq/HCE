...
namespace {
	...
	void run_bench() {
		resize_tt(64); 
		const i32 bench_depth = 13;
		u64 total_nodes = 0;
		auto start_time = std::chrono::steady_clock::now();
		
		// Suppress search info output during benchmarking
		std::streambuf* old_cout = std::cout.rdbuf();
		std::ostringstream suppressed;
		std::cout.rdbuf(suppressed.rdbuf());
		
		for (const char* fen : bench_positions) {
			Position pos;
			pos.set_fen(fen);
			clear_tt();
			u64 nodes = 0;
		...
		i64 elapsed_ms = std::max<i64>(1,
			std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time).count());
		u64 nps = total_nodes * 1000ULL / static_cast<u64>(elapsed_ms);
